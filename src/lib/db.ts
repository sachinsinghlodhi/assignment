...
export function getFacets(): SearchResponse["facets"] {
  ...
  return { categories, countries, techCategories, technologies };
}
...
export function searchCompanies(filters: SearchFilters): SearchResponse {
  ...
  // Free-text search on name + domain
  if (filters.search) {
  ...
  // AND technologies: company must have ALL of these
  ...
  // OR technologies: company must have at least ONE of these
  ...
  // NOT technologies: company must NOT have any of these
  ...
  // Tech category filter: company uses at least one tech from these categories
  ...
  // Per-category tech count: "2+ technologies in Advertising"
  // Uses the pre-computed company_tech_categories table (no runtime join)
  if (
  ...
  // Fetch technologies for each result row
  const getTechsStmt = db.prepare(
    "SELECT tech_name FROM company_technologies WHERE domain = ? ORDER BY tech_name",
  );

  const results: CompanyResult[] = rows.map((row) => ({
    ...
    technologies: (
      getTechsStmt.all(row.domain as string) as { tech_name: string }[]
    ).map((t) => t.tech_name),
  }));
...
export function exportAllCompanies(filters: SearchFilters): CompanyResult[] {
  ...
  const rows = db
    .prepare(`SELECT c.* FROM companies c ${whereClause} ORDER BY c.domain ASC`)
